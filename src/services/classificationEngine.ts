...
import type { BranchDefinition, ClassificationOptions, ClassificationResult, ClassifiedKeyword, KeywordInputItem } from '@/types/keyword';
...
export const classifyKeywords = (
  ...
  for (const item of keywordItems) {
    const original = item.keyword.trim();
    if (!original) {
      continue;
    }
  ...
  return {
    ...
    unknown,
    stats: {
      totalInput: keywordItems.length,
